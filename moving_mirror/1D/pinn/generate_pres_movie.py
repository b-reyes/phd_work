...
nn_pred_u = np.zeros((N_f + 1, M_f+1))
nn_pred_v = np.zeros((N_f + 1, M_f+1))
exact_u = np.zeros((N_f + 1, M_f+1))
exact_v = np.zeros((N_f + 1, M_f+1))

all_xpnts = np.zeros((N_f + 1, M_f+1))
...
for i in range(M_f+1):
    ...
    nn_pred_u[:, i] = pred_u.flatten()
    nn_pred_v[:, i] = pred_v.flatten()
    Exact_full = exact_val(x_pnts1, np.ones_like(x_pnts1)*t_exact[i], beta, acon, k, w, PI, k_c)
    exact_u[:, i] = Exact_full[:, 0:1].flatten()
    exact_v[:, i] = Exact_full[:, 1:2].flatten()


mdic = {"exact_u": exact_u, "exact_v": exact_v, "nn_pred_u": nn_pred_u, "nn_pred_v": nn_pred_v, "all_xpnts": all_xpnts,
        "times": times}

savemat("1d_matlab_movie_data_k_" + str(int(k)) + ".mat", mdic)
