...
nn_pred_u = np.zeros((N_f_x + 1, N_f_y + 1, N_f_z + 1, 3))
nn_pred_v = np.zeros((N_f_x + 1, N_f_y + 1, N_f_z + 1, 3))
...
nn_pred_u[:, :, :, 0] = pred_u.reshape((N_f_x+1, N_f_y+1, N_f_z+1))
nn_pred_v[:, :, :, 0] = pred_v.reshape((N_f_x+1, N_f_y+1, N_f_z+1))
...
h_exact_y = (acony * t_exact[i] ** 2 - betay) / N_f_y
h_exact_z = (aconz * t_exact[i] ** 2 - betaz) / N_f_z
x_pnts2 = betax + n_x*h_exact_x
y_pnts2 = betay + n_y*h_exact_y
z_pnts2 = betaz + n_z*h_exact_z
x_exact, y_exact, z_exact = np.meshgrid(x_pnts2, y_pnts2, z_pnts2)
net_x = x_exact.reshape(((N_f_x+1)*(N_f_y+1)*(N_f_z+1), 1))
net_y = y_exact.reshape(((N_f_x + 1)*(N_f_y + 1)*(N_f_z+1), 1))
net_z = z_exact.reshape(((N_f_x + 1)*(N_f_y + 1)*(N_f_z+1), 1))
pred_u, pred_v = model.predict(net_x, net_y, net_z, np.ones_like(net_x)*t_exact[i])
nn_pred_u[:, :, :, 1] = pred_u.reshape((N_f_x+1, N_f_y+1, N_f_z+1))
nn_pred_v[:, :, :, 1] = pred_v.reshape((N_f_x+1, N_f_y+1, N_f_z+1))
...
h_exact_y = (acony * t_exact[i] ** 2 - betay) / N_f_y
h_exact_z = (aconz * t_exact[i] ** 2 - betaz) / N_f_z
x_pnts3 = betax + n_x*h_exact_x
y_pnts3 = betay + n_y*h_exact_y
z_pnts3 = betaz + n_z*h_exact_z
x_exact, y_exact, z_exact = np.meshgrid(x_pnts3, y_pnts3, z_pnts3)
net_x = x_exact.reshape(((N_f_x+1)*(N_f_y+1)*(N_f_z+1), 1))
net_y = y_exact.reshape(((N_f_x + 1)*(N_f_y + 1)*(N_f_z+1), 1))
net_z = z_exact.reshape(((N_f_x + 1)*(N_f_y + 1)*(N_f_z+1), 1))
pred_u, pred_v = model.predict(net_x, net_y, net_z, np.ones_like(net_x)*t_exact[i])
nn_pred_u[:, :, :, 2] = pred_u.reshape((N_f_x+1, N_f_y+1, N_f_z+1))
nn_pred_v[:, :, :, 2] = pred_v.reshape((N_f_x+1, N_f_y+1, N_f_z+1))

mdic = {"nn_pred_u": nn_pred_u, "nn_pred_v": nn_pred_v, "x_pnts1": x_pnts1, "x_pnts2": x_pnts2, "x_pnts3": x_pnts3,
        "y_pnts1": y_pnts1, "y_pnts2": y_pnts2, "y_pnts3": y_pnts3, "z_pnts1": z_pnts1, "z_pnts2": z_pnts2,
        "z_pnts3": z_pnts3, "times": times}

savemat("3d_matlab_data_k_" + str(int(kx)) + ".mat", mdic)
