...
class PINN:
    ...
    def __init__(self, XYT_exact, u_layers, v_layers, betax, betay, aconx, acony, alpha, kx, ky, w, k_c, iters, uDir='', vDir=''):
        ...
        self.t_c_tf = tf.placeholder(tf.float32, shape=[None, 1])

        self.force_u_tf = tf.placeholder(tf.float32, shape=[None, 1])
        self.force_v_tf = tf.placeholder(tf.float32, shape=[None, 1])

        self.x_exact_tf = tf.placeholder(tf.float32, shape=[None, self.x_exact.shape[1]])
        self.y_exact_tf = tf.placeholder(tf.float32, shape=[None, self.y_exact.shape[1]])
        self.t_exact_tf = tf.placeholder(tf.float32, shape=[None, self.t_exact.shape[1]])
    ...
    def load_NN(self, fileDir, layers):
        ...
        with open(fileDir, 'rb') as f:
            ...
            for num in range(0, num_layers - 1):
                ...
                weights.append(W)
                biases.append(b)
            mm = tf.Variable(uv_multiplier[0], dtype=tf.float32)
