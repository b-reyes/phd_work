...
nn_pred_u = np.zeros((N_f_x + 1, N_f_y + 1, 3))
nn_pred_v = np.zeros((N_f_x + 1, N_f_y + 1, 3))
...
nn_pred_u[:, :, 0] = pred_u.reshape((N_f_x+1, N_f_y+1))
nn_pred_v[:, :, 0] = pred_v.reshape((N_f_x+1, N_f_y+1))
...
net_y = y_exact.reshape(((N_f_x + 1) * (N_f_y + 1), 1))
pred_u, pred_v = model.predict(net_x, net_y, np.ones_like(net_x)*t_exact[i])
nn_pred_u[:, :, 1] = pred_u.reshape((N_f_x+1, N_f_y+1))
nn_pred_v[:, :, 1] = pred_v.reshape((N_f_x+1, N_f_y+1))
...
net_y = y_exact.reshape(((N_f_x + 1) * (N_f_y + 1), 1))
pred_u, pred_v = model.predict(net_x, net_y, np.ones_like(net_x)*t_exact[i])
nn_pred_u[:, :, 2] = pred_u.reshape((N_f_x+1, N_f_y+1))
nn_pred_v[:, :, 2] = pred_v.reshape((N_f_x+1, N_f_y+1))

mdic = {"nn_pred_u": nn_pred_u, "nn_pred_v": nn_pred_v, "x_pnts1": x_pnts1, "x_pnts2": x_pnts2, "x_pnts3": x_pnts3,
        "y_pnts1": y_pnts1, "y_pnts2": y_pnts2, "y_pnts3": y_pnts3, "times": times}

savemat("2d_matlab_data_k_" + str(int(kx)) + ".mat", mdic)
